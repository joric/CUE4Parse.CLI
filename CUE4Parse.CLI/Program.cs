...
internal static class Program
{
    private static async Task<int> Main(string[] args)
    {
        ...
        var format      = new Option<string>  (new[] { "-f", "--format" },      ()=> "auto", "Output format: raw, json, csv, png");
        ...
        string game, string[] keys, string? mappings, string format, bool list, bool overwrite, bool verbose)
    {
...
#endif
        {
            ...
            if (targetFormat != "raw")
            {
                ...
                {
                    case ".umap":
                        if (targetFormat == "png") { // allow heightmap export
                            break;
                        }
                        targetFormat = "json";
                        break;
            ...
            // types below are only auto and texture
            if (targetFormat != "auto" && targetFormat != "png") {
                counter++;
                return;
            }

            if (targetFormat == "png") {
                type = ExportType.Texture; // only allow textures
            }
